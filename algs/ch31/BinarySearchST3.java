...
/**
 * Created by mitya on 11/2/16.
 */
public class BinarySearchST3<Key extends Comparable<Key>, Value> implements ST<Key, Value> {
    private Item [] items;
    private int n;
    ...
    public int rank(Key key) {
        Key tKey = null;
        if(items[lastAccess] != null)
            tKey = (Key) items[lastAccess].getKey();
        if(tKey != null && key.compareTo(tKey) == 0) {
            return lastAccess;
        }
        int lo = 0, hi = n - 1;
        while(lo <= hi) {
    ...
    public Value get(Key key) {
        //if(key.compareTo((Key) items[lastAccess].getKey()) == 0) return (Value)items[lastAccess].getValue();
