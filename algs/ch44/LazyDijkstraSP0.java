...
public class LazyDijkstraSP0 {
    ...
    private class ByDistanceFromSource implements Comparator<DirectedEdge> {
        ...
        public int compare(DirectedEdge e, DirectedEdge f) {
            double dist0 = distTo[e.from()] + e.weight();
            double dist1 = distTo[f.from()] + f.weight();
            return Double.compare(dist0, dist1);
        }
    }

    // single-source shortest path problem from s
    public LazyDijkstraSP0(EdgeWeightedDigraph G, int s) {
    ...
    public static void main(String[] args) {
        ...
        int s = 0;
        LazyDijkstraSP0 spt = new LazyDijkstraSP0(G, s);
        StdOut.println();
